...
The desktop UI runs locally on the device that starts it. The Onecurl installer places `Agent Vault.app` in `~/Applications`; `agent-vault-sync ui` opens that macOS app, which starts a private local UI bridge. The app also adds a macOS menu bar status icon. Closing the window keeps Agent Vault active from the menu bar; use the menu for `Open Window`, `Sync Now`, `Turn Auto-Sync On/Off`, `Refresh Status`, Mac Mini server/client visibility, recent flow stats, or `Quit`. Shared-folder registrations live under `~/.agent-vault/shares.json`, preferences live under `~/.agent-vault/preferences.json`, the Vault token stays out of the UI bundle, and each shared folder mirrors under its own Vault prefix so multiple folders do not collide. Adding a share triggers one initial sync. After that, normal daily mode is the menu bar app: Auto-Sync is optional and file-event driven. When enabled it reacts to added, moved or edited files in shared folders and debounces one sync pass; it does not run startup sync, timed remote sync, UI-refresh sync, or automatic edited-copy writeback. `Sync Now` and per-folder sync remain manual, and `Save Edits` is the only writeback path for opened edit copies. `agent-vault-sync watch` remains a terminal debug fallback, not the daily way to keep Agent Vault alive. Share prefixes are ignored by the main `~/AgentVault` sync to avoid duplicate pulls. Empty shared folders are represented by a hidden `.agent-vault-folder` marker. Use `agent-vault-sync ui --browser` only for debug.
