...
The desktop UI runs locally on the device that starts it. The Onecurl installer places `Agent Vault.app` in `~/Applications`; `agent-vault-sync ui` opens that macOS app, which starts a private local UI bridge. The app also adds a macOS menu bar status icon. Closing the window keeps Agent Vault active from the menu bar; use the menu for `Open Window`, `Sync Now`, `Refresh Status`, Mac Mini server/client visibility, recent flow stats, or `Quit`. Shared-folder registrations live under `~/.agent-vault/shares.json`, the Vault token stays out of the UI bundle, and each shared folder mirrors under its own Vault prefix so multiple folders do not collide. Adding a share triggers an initial sync. After that, normal daily mode is the menu bar app: it uses file-system events for local changes, debounces sync, caches unchanged file hashes, remote-polls slowly, and pauses visible UI refreshes when the window is closed. `agent-vault-sync watch` remains a terminal debug fallback, not the daily way to keep Agent Vault alive. Share prefixes are ignored by the main `~/AgentVault` sync to avoid duplicate pulls. Empty shared folders are represented by a hidden `.agent-vault-folder` marker. Use `agent-vault-sync ui --browser` only for debug.
