...
Device visibility is separate from token creation. After the presence-enabled server is running, authenticated API calls update `lastSeenAt`; the desktop app reads `/devices/status` and shows the Mac Mini Vault server plus MacBook, phone and Agent Desk clients as online/recent/offline with their allowed spaces. Tokens are never exposed through that status endpoint.
...
Conflicts are written under `~/AgentVault/.agent-vault/conflicts` instead of overwriting edits. `watch` follows `~/AgentVault` and every enabled shared folder in `~/.agent-vault/shares.json`, but only reacts to filesystem events.

The desktop UI runs locally on the device that starts it. The private DMG is served at `https://mac-mini-von-nils.tail8ca788.ts.net:8476/install/Agent-Vault.dmg`; the Onecurl installer configures the client and installs `Agent Vault.app` into `/Applications` when possible, falling back to `~/Applications` only without write access. `agent-vault-sync ui` opens that macOS app, which starts a private local UI bridge from the app bundle. The app also adds a macOS menu bar status icon. Closing the window keeps Agent Vault active from the menu bar; use the menu for `Open Window`, `Sync Now`, `Turn Auto-Sync On/Off`, `Refresh Status`, Mac Mini server/client visibility, recent flow stats, or `Quit`. Shared-folder registrations live under `~/.agent-vault/shares.json`, preferences live under `~/.agent-vault/preferences.json`, the Vault token stays out of the UI bundle, and each shared folder mirrors under its own Vault prefix so multiple folders do not collide. Adding a share uses the native macOS folder picker and queues one initial sync without blocking the UI. After that, normal daily mode is the menu bar app: Auto-Sync is optional and file-event driven. When enabled it reacts to added, moved or edited files in shared folders and debounces one sync pass; it does not run startup sync, timed remote sync, UI-refresh sync, or automatic edited-copy writeback. `Sync Now` and per-folder sync remain manual, and `Save Edits` is the only writeback path for opened edit copies. The Finder-style browser opens local and remote folders through lazy folder listings rather than capped summary trees, so large folders such as Desktop are not represented by a truncated preview. `agent-vault-sync watch` remains a terminal debug fallback, not the daily way to keep Agent Vault alive. Share prefixes are ignored by the main `~/AgentVault` sync to avoid duplicate pulls. Empty shared folders are represented by a hidden `.agent-vault-folder` marker. Use `agent-vault-sync ui --browser` only for debug.
