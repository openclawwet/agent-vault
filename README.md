...
On first launch, the app can create its local sync config by contacting the private Mac Mini Tailnet install endpoint and storing the scoped MacBook token locally. The curl installer remains only an update/debug path, not the daily way to run Agent Vault.
